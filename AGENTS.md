...
- **类型**：CS2 客户端 .cfg 配置合集（不是代码项目，没有构建/测试/lint 流水线）
- **用户**：个人自用精简版本
...
├── README.en.md                           # 英文 README（GitHub 自动识别双语）
├── assets/                                # 静态资源（cs2-icon、键位图）
│   ├── cs2-icon.png
│   ├── cs2-icon-rounded.png
│   └── 键位图.png
└── cfg/                                   # 实际配置（被 .gitignore 之外的内容跟踪）
...
仓库本身**没有构建/测试/lint** 步骤。配置直接拷贝到 `game\csgo\cfg\` 目录使用。
...
游戏自动生成的 .cfg、录像、缓存都不会进版本库。`cfg/config/VNL/deprecated/` 通过 `**` 隐式跟踪（保留历史参考）。
